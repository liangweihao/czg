...
*/
    ...
    static String num = "[1][3456789]\\d{9}";//"[1]"代表第1位为数字1，"[358]"代表第二位可以为3、5、8中的一个，"\\d{9}"代表后面是可以是0～9的数字，有9位。
    String mobPhnNum;
...
// 判断手机号码是否符合中国移动的号码规则
            if (mobPhnNum.matches(num)) {
                flag = 1;
            }
// 判断手机号码是否符合中国联通的号码规则
            else if (mobPhnNum.matches(num)) {
                flag = 2;
            }
// 判断手机号码是否符合中国电信的号码规则
            else if (mobPhnNum.matches(num)) {
...
// 不是11位
    ...
    public static boolean isValidPhoneNumber(String number) {
        boolean flag = false;
        if (number.length() == 11 && (number.matches(num) || number.matches(num) || number.matches(num)))
        {
            flag = true;
        }
        return flag;
    ...
    public static boolean isEmail(String email) {
        String patternString = "^[//w//.-] aliyunzixun@xxx.com([//w//-]+//.)+[A-Z]{2,4}$";
