...
public class ProjectLibraryRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private static final int TYPE_ITEM1 = 1;  //左右结构布局
    private static final int TYPE_ITEM2 = 2;  //上下结构布局
    private List<LibraryProjectList.ResponseParamsBean> list;
    private Context context;
    private int type;   //判断  条目类型

    public ProjectLibraryRecyclerAdapter(Context productionFragment, List<LibraryProjectList.ResponseParamsBean> mDatas, int type) {
        this.context = productionFragment;
        this.list = mDatas;
        this.type = type;
    }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        final LibraryProjectList.ResponseParamsBean.ProjectDataBean dataBean = list.get(position).getProjectData();
        //item---->左右结构
        if (holder instanceof Item1ViewHolder) {
            ...
            if (list.get(position).getCollection() == 1){
                //有红色心---已收藏
                ((Item1ViewHolder) holder).ivProjectlibraryItem1Collect.setVisibility(View.VISIBLE);  //收藏
            }else if (list.get(position).getCollection() == 0){
                //无红色心---未收藏
                ((Item1ViewHolder) holder).ivProjectlibraryItem1Collect.setVisibility(View.INVISIBLE);  //未收藏
        ...
        if (holder instanceof Item2ViewHolder) {
            ...
            if (list.get(position).getCollection() == 1){
                //有红色心---已收藏
                ((Item2ViewHolder) holder).ivProjectlibraryItem2Collect.setVisibility(View.VISIBLE);  //收藏
            }else if (list.get(position).getCollection() == 0){
                //无红色心---未收藏
                ((Item2ViewHolder) holder).ivProjectlibraryItem2Collect.setVisibility(View.INVISIBLE);  //未收藏
            }
            //收藏点击事件
